...
const Sidebar = () => {
    ...
    const { logout, onlineUser, token } = useContext(AuthContext);

    const [input, setInput] = useState(false);

    const navigate = useNavigate();

    const safeUser = Array.isArray(user) ? user : [];
    const filterUser = input ? safeUser.filter((u) => u.fullName.toLowerCase()
        .includes(input.toLowerCase())) : safeUser;
